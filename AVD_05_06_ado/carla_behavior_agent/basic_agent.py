...
class BasicAgent(object):
    ...
    def lane_change_left(self, direction, same_lane_time=0.1, other_lane_time=6, lane_change_time=1):
        """
    ...
    def _vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        ...
        for target_vehicle in vehicle_list:
            ...
            # per gestire il bug delle bici dove nel primo scenario le bici hanno lane id pari a 2 e lane type uguale a Shoulder
            if target_wpt.lane_type == carla.LaneType.Shoulder and ego_wpt.lane_id * target_wpt.lane_id > 0 and target_wpt.road_id == ego_wpt.road_id:
                
                self._count_vehicles += 1
                ...
                if is_within_distance(target_rear_transform, ego_front_transform, max_distance, [low_angle_th, up_angle_th]):
                    list_of_vehicles[compute_distance(target_transform.location, ego_transform.location)] = [target_vehicle, target_wpt.lane_id, target_wpt.lane_type]
        ...
        if(len(list_of_vehicles) == 0): 
            return (False, None, -1, None, None, 0)
        else:
            distance = min(list_of_vehicles.keys())
            return (True, list_of_vehicles[distance][0], distance, list_of_vehicles[distance][1], list_of_vehicles[distance][2], self._count_vehicles)
