    ...
    def _vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        ...
        for target_vehicle in vehicle_list:
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                ...
                target_forward_vector = target_transform.get_forward_vector()
                target_extent = target_vehicle.bounding_box.extent.x
                target_rear_transform = target_transform        #parte posteriore del veicolo
                target_rear_transform.location -= carla.Location(
                    x=target_extent * target_forward_vector.x,
                    y=target_extent * target_forward_vector.y,
                )
            ...
            # negli incroci non posso escludere oggetti che sono su altre strade o corsie
                ...
                # costruisco un poligono che include il mio veicolo
                route_bb = []
                ego_location = ego_transform.location
                extent_y = self._vehicle.bounding_box.extent.y
                r_vec = ego_transform.get_right_vector()
                ...
                for wp, _ in self._local_planner.get_plan():
                    if ego_location.distance(wp.transform.location) > max_distance: # esculdo i punti del mio persorso più lontani
                        break
                    
                    # per ogni punto della traittoria costruisco un poligono considerando il punto più a destra e più a sinistra del mio veicolo
                    r_vec = wp.transform.get_right_vector()
                ...
                for target_vehicle in vehicle_list:
                    ...
                    # costruisco il poligono anche per il veicolo considerato
                    target_bb = target_vehicle.bounding_box
                    target_vertices = target_bb.get_world_vertices(target_vehicle.get_transform())
                    target_list = [[v.x, v.y, v.z] for v in target_vertices]
                    target_polygon = Polygon(target_list)

                    # controllo per possibili intersezioni tra i due poligoni
                    # (si potrebbe controllare una posizione futura del veciolo facendo una proiezione in base alla sua direzione)
